...
//You can transform the number on screen to the stat here, using the function at the bottom of this script
    ...
    public void DoAJump(bool overrideChecks = false)
    {

        //Create the jump, provided we are on the ground, in coyote time, or have a double jump available
        bool coyoteJump = (coyoteTimeCounter > 0.03f && coyoteTimeCounter < coyoteTime);
        if (onGround || coyoteJump || canJumpAgain || overrideChecks)
        {
            desiredJump = false;
            jumpBufferCounter = 0;
            coyoteTimeCounter = 0;
